...
Each area's list of possible download locations is resolved from its `chat.*` configuration key (e.g., `chat.agentFilesLocations` for agents, `chat.pluginLocations` for plugins, `chat.agentSkillsLocations` for skills). These settings are normally object maps in the format `{ "path/1": true, "path/2": false }`, where `false` disables a location and only enabled locations (where the value is not `false`) are included. For backward compatibility, the parser also accepts the legacy array format (`["path/1", "path/2"]`) and treats listed paths as enabled. If the config key is not set or contains no enabled locations, a default list is generated from 8 template prefixes (`{.agents,.claude,.github,.kiro,~/.agents,~/.claude,~/.copilot,~/.kiro}/{area}`). Hooks - Kiro is fixed to `.kiro/hooks`.

Related external settings: each area view scans locations from its own `chat.*` setting (e.g. `chat.agentFilesLocations` for agents, `chat.pluginLocations` for plugins), using enabled entries from the object-map form or listed paths from the legacy array form. When the setting isn't configured or has no enabled locations, a default list is generated from the template prefixes. The configured download location from `AIToolsOrganizer.installLocations` is always included in the scan.
...
| `SkillInstallationService` | Downloads, deletes, moves, copies, syncs skills. Uses area-specific download locations based on `skill.area`. Handles overwrite confirmation, progress notifications, and resilient deletion via shared trash-first fallback behavior. Success is silent — notifications are only shown on failure. |
