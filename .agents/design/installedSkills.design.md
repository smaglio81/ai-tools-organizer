...
The Installed Skills view (`AIToolsOrganizer.skills`) shows all skills currently installed on the machine — both in workspace-relative folders and in user home directory folders. It is the primary interface for managing locally installed skills.
...
The setting supports two formats: an object map (`{ "path/1": true, "path/2": false }`) where paths set to `false` are excluded, and a legacy string array where all listed paths are treated as enabled. If all configured paths are disabled (or the setting is empty), the generated default locations below are used as a fallback.
...
All actions appear in the `AIToolsOrganizer.skills` view title bar.

| Button | Command | Description |
|---|---|---|
| Search (search icon) | `AIToolsOrganizer.searchInstalled` | Opens an input box; filters displayed skills by name or description (case-insensitive). |
| Clear Search (close icon) | `AIToolsOrganizer.clearSearchInstalled` | Clears the active search filter. Only shown when a search is active (`AIToolsOrganizer:installedSearchActive` context key). |
| Refresh | `AIToolsOrganizer.refreshInstalled` | Re-scans all skill locations and refreshes the tree. Also syncs installed skill names back to the Marketplace view. |
| Default Download Location (folder icon) | `AIToolsOrganizer.selectInstallLocation` | Opens a QuickPick to change the skills download location in `AIToolsOrganizer.installLocations`. Selecting "Custom..." opens the VS Code Settings UI filtered to `AIToolsOrganizer.installLocations`. |
| Add Skill (add icon) | `AIToolsOrganizer.newSkillItem` | Opens a location QuickPick, prompts for a name, and creates a new skill folder with `SKILL.md` scaffolding. |
| Expand All | `AIToolsOrganizer.expandAll` | Expands all location groups. Uses `TreeView.reveal()` (requires `getParent()` implementation). |
| Collapse All | `AIToolsOrganizer.collapseAll` | Collapses all location groups. Delegates to the built-in `workbench.actions.treeView.AIToolsOrganizer.skills.collapseAll` command. |
...
- The `AIToolsOrganizer:installedSearchActive` VS Code context key is set to `true` when a query is active, enabling the Clear Search button.
...
|---|---|---|---|
| Add File | `AIToolsOrganizer.addFile` | Always | Prompts for a file name, creates an empty file inside the skill folder, and opens it in the editor. |
| Add Folder | `AIToolsOrganizer.addFolder` | Always | Prompts for a folder name and creates a new subfolder inside the skill folder. |
| Copy #{path} | `AIToolsOrganizer.copyItemPath` | Always | Copies a chat-ready path reference like `#~/.copilot/skills/my-skill` (or nested file/folder path) to the clipboard. |
| Copy Absolute Path | `AIToolsOrganizer.copyAbsolutePath` | Always | Copies the fully resolved filesystem path to the clipboard. |
| Reveal in File Explorer | `AIToolsOrganizer.revealInFileExplorer` | Always | Opens the skill folder in the system file explorer. |
| Move to... | `AIToolsOrganizer.moveSkill` | Always | Opens a QuickPick listing all scan locations (current location marked). Moves the skill folder to the selected location. |
| Copy to... | `AIToolsOrganizer.copySkill` | Always | Opens a QuickPick listing all scan locations (current location marked). Copies the skill folder to the selected location, keeping the original. |
| Copy to Plugin... | `AIToolsOrganizer.copyToPlugin` | Always | Opens a QuickPick listing all installed plugins. Copies the skill into the selected plugin's `skills/` subfolder. |
| Update Plugins | `AIToolsOrganizer.updatePlugins` | Always | Searches all installed plugins for a copy of this skill in their `skills/` subfolder and overwrites each found copy with the current version. Results shown via output channel. |
| Copy Name | `AIToolsOrganizer.copyItemName` | Always | Copies the skill name to the clipboard. |
| Duplicate | `AIToolsOrganizer.duplicateItem` | Always | Prompts for a new name and creates a copy of the skill in the same location, updating the definition file name. |
...
| Menu Item | Command | Description |
|---|---|---|
| Add File | `AIToolsOrganizer.addFile` | Prompts for a file name, creates an empty file inside the subfolder, and opens it in the editor. |
| Add Folder | `AIToolsOrganizer.addFolder` | Prompts for a folder name and creates a new subfolder. |
...
| Menu Item | Command | Description |
|---|---|---|
| Copy #{path} | `AIToolsOrganizer.copyItemPath` | Copies a chat-ready path reference for the file to the clipboard. |
...
The collapsed/expanded state of each location group is persisted in `workspaceState` under the key `AIToolsOrganizer.collapsedLocations` (stored as a `string[]` of collapsed location paths).
