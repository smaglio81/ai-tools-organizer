...
The setting supports two formats: an object map (`{ "path/1": true, "path/2": false }`) where paths set to `false` are excluded, and a legacy string array where all listed paths are treated as enabled.
